  ...
  const { data, error, isLoading, mutate } = useSWR<HealthPlanInterface>(
    ...
    () =>
      getHealthPlanById(id, {
        relations: ['user_health_plan_senior_user_idTouser', 'user_health_plan_health_coach_idTouser', 'progress'],
      }),
  );
            ...
            <Box>
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Health Plan Senior User Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_health_plan_senior_user_idTouser?.id}`}>
                      {data?.user_health_plan_senior_user_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Health Plan Health Coach Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_health_plan_health_coach_idTouser?.id}`}>
                      {data?.user_health_plan_health_coach_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('progress', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/progresses/create?health_plan_id=${data?.id}`}>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.progress?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/progresses/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('progress', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/progresses/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('progress', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    progressHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
