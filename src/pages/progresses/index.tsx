...
function ProgressListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('progress', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/progresses/view/${id}`);
    }
  };
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('progress', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            ...
            <NextLink href={`/progresses/create`} passHref legacyBehavior>
                    ...
                    {hasAccess('health_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/health-plans/view/${record.health_plan?.id}`}>
                          {record.health_plan?.plan_details}
                        </Link>
                      </Td>
                    )}
